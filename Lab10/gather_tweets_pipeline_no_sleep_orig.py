...
class MyListener(StreamListener):
    ...
    def on_data(self, data):
        ...
        increment=1
        if len(str(self.max_num))>2:
            increment=self.max_num//10
        ...
        with open(self.raw_file, 'a') as f_raw, open(self.csv_file, 'a') as f_csv, open(self.text_file, 'a') as f_text:
            ...
            if (self.count % increment == 0 or self.count==1) and self.count > 0 and self.count<=self.max_num:
                sys.stdout.write('\n{}/{} tweets downloaded'.format(self.count, self.max_num))#\r
                sys.stdout.flush()
            ...
            if self.count == self.max_num:
                ...
                print('It took {} seconds to download {} tweets'.format(format(elapse,'.3f'), self.max_num),'\n')
                return False
...
def main():
    ...
    consumer_key = 'QTQ986igfoA25rsWtdlmjJ12b'
    consumer_secret = '4MGsrTbcFuBNHjRdWekbQ83pTdTIzE0l0KAylbZEvRp96K97hs'
    access_token = '2374447776-y4nYBkCe4XwjBOogUVQKhnKwKyi3rlxy238e3mH'
    access_secret = 'dg9AD9g4nBXNiLVL5bC7Ugowu6EhPSnvji6aVIILsaUia'
    ...
    if m == 'file':
        ...
        while True:
            line = input('Two words can be a single key word. >>> ')
            line = line.strip()
            invalid_flag = False
    ...
    print('{} unique key words being used: '.format(len(key_words)), key_words,'\n')
    ...
    while True:    
        ...
        print('Would you like to download the key words automatically over time?')
        cycle_answer=input('Input "Yes" or "No" >>>')
        possible_answer1=['Yes','YES','yes','y','Y','yEs','YEs']
        possible_answer2=['No','NO','no','N','n']
        if cycle_answer in possible_answer1:
            print()
            print('Would you like to download by Hours or Minutes?')
            cycle_time=input('Input "Hour" or "Minute" >>> ')
            print()
            possible_answer3=['Hour','HOUR','hour']
            possible_answer4=['Minute','MINUTE','minute']
            if (cycle_time in possible_answer3) or (cycle_time in possible_answer4):
                  cycle_time_int=input('Enter a numerical amount for '+cycle_time+' >>> ')
                  print()
                  cycle_amount=input('''Enter the number of times you wish to cycle.
Press <ENTER> for continuous >>> ''')
                  if cycle_time_int.isdigit() and (cycle_amount.isdigit() or cycle_amount==''):
                      count=0
                      var_time=time.ctime()[11:19]
                      while True:
                          if time.ctime()[11:19]==var_time or time.ctime()[11:19]==time.ctime()[11:14]+str(int(time.ctime()[14:16])+1)+time.ctime()[16:19] or time.ctime()[11:19]==str(int(time.ctime()[11:13])+1)+time.ctime()[13:19]:
                              if cycle_amount=='' or count!=int(cycle_amount):
                                  postfix = current_datetime_str()
                                  ...
                                  count+=1
                                  if cycle_time in possible_answer3 and (cycle_amount=='' or count!=int(cycle_amount)):
                                      print('You can press "Ctrl+C" or "Exit" at anytime to abort the program.')
                                      print('Waiting for',cycle_time_int,cycle_time+'s ')
                                      var_time=str((int(time.ctime()[11:13])+int(cycle_time_int))%24)+time.ctime()[13:19]
                                      #time.sleep(3600*int(cycle_time_int))
                                  elif cycle_amount=='' or count!=int(cycle_amount):
                                      print('You can press "Ctrl+C" or "Exit" at anytime to abort the program.')
                                      print('Waiting for',cycle_time_int,cycle_time+'s ')
                                      var_time=time.ctime()[11:14]+str((int(time.ctime()[14:16])+int(cycle_time_int))%60)+time.ctime()[16:19]
                                      #time.sleep(60*int(cycle_time_int))
                              else:
                                  print('\nDONE\n')
                                  sys.exit(0)
                                  break
                  else:
                      print('\nEnter the Proper Comands\n')
                
        elif cycle_answer in possible_answer2:
            print('Start gathering tweets ...')
            raw_file = 'raw_{}.json'.format(postfix)
            csv_file = 'data_{}.csv'.format(postfix)
            text_file = 'text_{}.csv'.format(postfix)
